...
class Trainer:
    ...
    def _plot_confusion_matrix(self, gr_th, pred, save_path, ratio_threshold=0.1):
        """Vẽ confusion matrix (tỉ lệ theo hàng). Chỉ hiện số trong ô khi tỉ lệ > ratio_threshold."""
        num_classes = self.cfg['model']['num_classes']
        cm = confusion_matrix(gr_th, pred, labels=list(range(num_classes)))
        row_sums = cm.sum(axis=1, keepdims=True)
        cm_norm = np.divide(cm, row_sums, out=np.zeros_like(cm, dtype=float), where=(row_sums > 0))
        n = num_classes
        # Chỉ hiện số khi tỉ lệ > ratio_threshold
        annot_mask = cm_norm > ratio_threshold
        annot_matrix = np.where(annot_mask, np.round(cm_norm, 2).astype(str), "")
        fig_size = min(120, max(10, n / 25))
        fig, ax = plt.subplots(figsize=(fig_size, fig_size))
        sns.heatmap(cm_norm, annot=annot_matrix, fmt="", cmap="Blues", ax=ax)
        ax.set_xlabel("Predicted")
        ax.set_ylabel("True")
        ax.set_title("Confusion matrix (normalized by row). Numbers shown if ratio > 0.1")
        if n > 100:
            ax.set_xticks([])
            ax.set_yticks([])
        plt.tight_layout()
    ...
    def evaluate_per_class(self, dataloader):
        ...
        dataset_name = self.cfg.get('data', {}).get('dataset_name', 'unknownz')
        self._plot_confusion_matrix(gr_th, pred, f"confusion_matrix_per_class_{dataset_name}.png")
    ...
    def evaluate_top_k_per_class(self, dataloader):
        ...
        dataset_name = self.cfg.get('data', {}).get('dataset_name', 'unknownz')
        self._plot_confusion_matrix(gr_th, pred, f"confusion_matrix_topk_per_class_{dataset_name}.png")
